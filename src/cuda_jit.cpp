...
struct nvrtc_api {
    using create_program_fn = nvrtc_result (*)(nvrtc_program*, const char*,
                                               const char*, int,
                                               const char* const*,
                                               const char* const*);
...
struct nvrtc_runtime {
    ...
    [[nodiscard]] explicit operator bool() const noexcept {
        return static_cast<bool>(library) && api.create_program != nullptr &&
               api.compile_program != nullptr && api.get_ptx_size != nullptr &&
               api.get_ptx != nullptr && api.get_log_size != nullptr &&
               api.get_log != nullptr && api.destroy_program != nullptr &&
...
struct cuda_api {
    ...
    using ctx_create_fn = cuda_result (*)(cuda_context*, uint32_t, cuda_device);
    ...
    using memcpy_htod_fn = cuda_result (*)(cuda_device_ptr, const void*,
                                           size_t);
    using memcpy_dtoh_fn = cuda_result (*)(void*, cuda_device_ptr, size_t);
    using module_load_data_fn = cuda_result (*)(cuda_module*, const void*);
    using module_unload_fn = cuda_result (*)(cuda_module);
    using module_get_function_fn = cuda_result (*)(cuda_function*, cuda_module,
                                                   const char*);
    using launch_kernel_fn = cuda_result (*)(cuda_function, uint32_t, uint32_t,
                                             uint32_t, uint32_t, uint32_t,
                                             uint32_t, uint32_t, cuda_stream,
                                             void**, void**);
    using ctx_synchronize_fn = cuda_result (*)();
    using get_error_name_fn = cuda_result (*)(cuda_result, const char**);
    using get_error_string_fn = cuda_result (*)(cuda_result, const char**);
...
struct cuda_runtime {
    ...
    [[nodiscard]] explicit operator bool() const noexcept {
        return static_cast<bool>(library) && api.init != nullptr &&
               ...
               api.ctx_set_current != nullptr && api.ctx_destroy != nullptr &&
               api.mem_alloc != nullptr && api.mem_free != nullptr &&
               api.memcpy_htod != nullptr && api.memcpy_dtoh != nullptr &&
               api.module_load_data != nullptr &&
               api.module_unload != nullptr &&
               api.module_get_function != nullptr &&
               api.launch_kernel != nullptr && api.ctx_synchronize != nullptr &&
               api.get_error_name != nullptr && api.get_error_string != nullptr;
    }
};
...
[[nodiscard]] nvrtc_runtime load_nvrtc() {
    ...
    nvrtc_api api;
    api.create_program =
        library.symbol<nvrtc_api::create_program_fn>("nvrtcCreateProgram");
    api.compile_program =
        library.symbol<nvrtc_api::compile_program_fn>("nvrtcCompileProgram");
    api.get_ptx_size =
        library.symbol<nvrtc_api::get_ptx_size_fn>("nvrtcGetPTXSize");
    api.get_ptx = library.symbol<nvrtc_api::get_ptx_fn>("nvrtcGetPTX");
    api.get_log_size =
        library.symbol<nvrtc_api::get_log_size_fn>("nvrtcGetProgramLogSize");
    api.get_log = library.symbol<nvrtc_api::get_log_fn>("nvrtcGetProgramLog");
    api.destroy_program =
        library.symbol<nvrtc_api::destroy_program_fn>("nvrtcDestroyProgram");
    api.get_error_string =
        library.symbol<nvrtc_api::get_error_string_fn>("nvrtcGetErrorString");
...
[[nodiscard]] cuda_runtime load_cuda_driver() {
    ...
    api.device_get = library.symbol<cuda_api::device_get_fn>("cuDeviceGet");
    api.ctx_create = library.symbol<cuda_api::ctx_create_fn>("cuCtxCreate_v2");
    api.ctx_set_current =
        library.symbol<cuda_api::ctx_set_current_fn>("cuCtxSetCurrent");
    api.ctx_destroy =
        library.symbol<cuda_api::ctx_destroy_fn>("cuCtxDestroy_v2");
    api.mem_alloc = library.symbol<cuda_api::mem_alloc_fn>("cuMemAlloc_v2");
    api.mem_free = library.symbol<cuda_api::mem_free_fn>("cuMemFree_v2");
    api.memcpy_htod =
    ...
    api.module_load_data =
        library.symbol<cuda_api::module_load_data_fn>("cuModuleLoadData");
    api.module_unload =
        library.symbol<cuda_api::module_unload_fn>("cuModuleUnload");
    api.module_get_function =
        library.symbol<cuda_api::module_get_function_fn>("cuModuleGetFunction");
    api.launch_kernel =
        library.symbol<cuda_api::launch_kernel_fn>("cuLaunchKernel");
    api.ctx_synchronize =
        library.symbol<cuda_api::ctx_synchronize_fn>("cuCtxSynchronize");
    api.get_error_name =
        library.symbol<cuda_api::get_error_name_fn>("cuGetErrorName");
    api.get_error_string =
        library.symbol<cuda_api::get_error_string_fn>("cuGetErrorString");

    return {std::move(library), api};
}

[[nodiscard]] std::string describe(const nvrtc_api& api, nvrtc_result result) {
...
[[nodiscard]] std::string describe(const cuda_api& api, cuda_result result) {
...
[[nodiscard]] std::string program_log(const nvrtc_api& api,
    ...
    check_nvrtc(api, api.get_log(program, log.data()), "nvrtcGetProgramLog");
    if (!log.empty() && log.back() == '\0') log.pop_back();
    return log;
}

[[nodiscard]] std::string cache_key(std::string_view source,
...
class program_handle {
  public:
    ...
    [[nodiscard]] nvrtc_program get() const noexcept { return program_; }
...
class device_memory {
  public:
    device_memory(const cuda_api& api, size_t bytes)
        ...
        if (bytes_ != 0)
            check_cuda(api_, api_.mem_alloc(&ptr_, bytes_), "cuMemAlloc");
    }
...
struct cuda_environment {
    cuda_environment() : runtime(load_cuda_driver()) {
        ...
        check_cuda(runtime.api, runtime.api.device_get(&selected_device, 0),
                   "cuDeviceGet");
...
std::string compile_to_ptx(std::string_view source,
    ...
    size_t ptx_size = 0;
    check_nvrtc(runtime.api, runtime.api.get_ptx_size(program.get(), &ptx_size),
                "nvrtcGetPTXSize");
...
void copy_to_device(device_buffer& buffer, std::span<const float32_t> source) {
    if (source.empty()) return;
    require_device_bytes(buffer, source.size_bytes(), "target");

    cuda_environment& env = environment();
    check_cuda(env.runtime.api,
               env.runtime.api.memcpy_htod(buffer.state_->ptr, source.data(),
                                           source.size_bytes()),
               "cuMemcpyHtoD");
}
...
void copy_to_host(const device_buffer& buffer, std::span<float32_t> target) {
    ...
    cuda_environment& env = environment();
    check_cuda(env.runtime.api,
               env.runtime.api.memcpy_dtoh(target.data(), buffer.state_->ptr,
                                           target.size_bytes()),
               "cuMemcpyDtoH");
}

void launch_elementwise_binary(std::string_view ptx, std::string_view function,
    ...
    launch_elementwise_binary(ptx, function, device_lhs, device_rhs, device_out,
                              out.size());
...
void launch_fused_elementwise(std::string_view ptx, std::string_view function,
